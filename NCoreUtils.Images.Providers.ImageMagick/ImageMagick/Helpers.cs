...
internal static class Helpers
{
    ...
    public static string? StringifyImageData(object? obj)
        => obj switch
        {
            ...
            Rational r => $"f64[{((double)r.Numerator / r.Denominator).ToString(CultureInfo.InvariantCulture)}]",
            Rational[] rs => $"f64[{string.Join(",", rs.Select(r => ((double)r.Numerator / r.Denominator).ToString(CultureInfo.InvariantCulture)))}]",
            _ => null
        };
}