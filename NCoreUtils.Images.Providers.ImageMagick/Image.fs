  ...
  let private magickFormatMap =
    Map.ofList
      [ MagickFormat.Jpg,    "jpg"
        ...
        MagickFormat.Png00,  "png"
        MagickFormat.Png8,   "png"
        MagickFormat.Png24,  "png"
        MagickFormat.Png32,  "png"
        MagickFormat.Png48,  "png"
        MagickFormat.Png64,  "png"
        MagickFormat.Bmp,    "bmp"
        MagickFormat.Bmp2,   "bmp"
        MagickFormat.Bmp3,   "bmp"
        MagickFormat.Gif,    "gif"
        MagickFormat.Gif87,  "gif"
        MagickFormat.Tiff64, "tiff"
  ...
  let private imageTypeMap =
    Map.ofList
      [ "jpg",  MagickFormat.Jpg
        ...
        "gif",  MagickFormat.Gif87
        "tiff", MagickFormat.Tiff ]

  let magickFormatToImageType format = Map.tryFind format magickFormatMap |> Option.defaultValue "other"
...
type ImageMagickImageProvider () =
  inherit ImageProvider ()

  member this.FromStream (stream : Stream) =
    new ImageMagickImage (new MagickImage (stream), this)

  override this.AsyncCreate stream =
...
and
  [<Sealed>]
  ImageMagickImage =
    val private provider           : ImageMagickImageProvider
    val private native             : MagickImage
    val private imageType          : string
    val mutable private size       : Size
    val mutable private isDisposed : int
    internal new (native : MagickImage, provider : ImageMagickImageProvider) =
      { isDisposed = 0
        provider   = provider
        native     = native
    ...
    member private this.IsDisposed
      with [<MethodImpl(MethodImplOptions.AggressiveInlining)>] get () = 0 <> this.isDisposed

    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member private this.ThrowIfDisposed () =
      if this.IsDisposed then ObjectDisposedException "Image" |> raise

    member this.Provider
      with [<MethodImpl(MethodImplOptions.AggressiveInlining)>] get () = this.provider

    member this.Size
      with [<MethodImpl(MethodImplOptions.AggressiveInlining)>] get () = this.size

    member this.ImageType
      with [<MethodImpl(MethodImplOptions.AggressiveInlining)>] get () = this.imageType
    ...
    member this.Resize ({ Width = width; Height = height } : Size) =
      this.ThrowIfDisposed ()
      this.native.Resize (width, height)
      this.size <- { Width = this.native.Width; Height = this.native.Height }
